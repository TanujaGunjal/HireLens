...
const { generateExperienceFromProjects } = require('./rewrite.service');
...
class ATSEngineAdapter {
  ...
  static resumeToEngineFormat(mongoResume) {
    ...
    return {
      _autoGeneratedExperience: mongoResume._autoGeneratedExperience === true || repo._autoGeneratedExperience === true,
   ...
   */
  static scoreResume(mongoResume, mongoJD, semanticScore = 0) {
    try {
      if (!mongoResume) throw new Error('Resume is required');
      if (!mongoJD) throw new Error('Job Description is required');

      // ── Step 0: Deep-clone so we never mutate the original Mongoose doc ────
      const rawDoc = mongoResume.toObject ? mongoResume.toObject() : mongoResume;
      const safeResume = JSON.parse(JSON.stringify(rawDoc));

      // ── Step 1: Inject auto-generated experience if empty (GLOBAL — runs for
      //            EVERY scoring path so base + rewrite scores are comparable) ──
      if (!safeResume._autoGeneratedExperience && (!safeResume.experience || safeResume.experience.length === 0)) {
        try {
          const autoExpResult = generateExperienceFromProjects(safeResume);
          const generated = autoExpResult
            ? (Array.isArray(autoExpResult) ? autoExpResult : autoExpResult.experience)
            : null;

          if (generated && generated.length > 0) {
            safeResume.experience = generated;
            safeResume._autoGeneratedExperience = true;
            console.log(`[ATS] Auto experience injected: ${generated.length} entries`);
          }
        } catch (expErr) {
          console.warn('[ATS] Auto experience generation failed:', expErr.message);
        }
      }
      ...
      const resume = this.resumeToEngineFormat(safeResume);
      if (!resume) throw new Error('Resume conversion failed');

      // Reflect the ACTUAL flag (after possible injection)
      const flag = safeResume._autoGeneratedExperience === true;
      console.log("[ATS] autoGeneratedExperience:", flag);
      console.log(`[ATS] experience count: ${resume.experience?.length || 0}`);
      ...
      if (!resumeHasContent) {
        console.error('❌ CRITICAL: Converted resume has no content in any section!');
        console.error('   Resume object:', JSON.stringify(resume, null, 2).substring(0, 500));
        throw new Error('Resume is completely empty — please add content before scoring.');
      }
      ...
      const semanticScore = mongoResume.semanticScore || 0;
      const result = ATSEngine.scoreResume(resume, jd, semanticScore);
